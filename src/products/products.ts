  ...
  switch (event.httpMethod) {
    case 'GET':
      ...
      if (segments.length === 0) {
        return ReadAll(event, dbClient, headers)
      }
      /* GET /.netlify/functions/api/123456 */
      if (segments.length === 1) {
        // event.id = segments[0];
        return Read(event, dbClient, segments[0], headers)
      } else {
        return {
          statusCode: 500,
    ...
    case 'POST':
      return Create(event, dbClient, headers)
      break
    /* PUT /.netlify/functions/api/123456 */
    case 'PUT':
      if (segments.length === 1) {
        // event.id = segments[0];
        return Update(event, dbClient, segments[0], headers)
      } else {
        return {
          statusCode: 500,
    ...
    case 'DELETE':
      if (segments.length === 1) {
        // event.id = segments[0];
        return Delete(event, dbClient, segments[0], headers)
      } else {
        return {
          statusCode: 500,
